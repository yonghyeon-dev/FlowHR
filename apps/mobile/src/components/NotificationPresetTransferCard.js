...
export default function NotificationPresetTransferCard({ presetState, onImportPresetState }) {
  const [payload, setPayload] = useState("");
  const [status, setStatus] = useState("");
  const [statusTone, setStatusTone] = useState("neutral");
  const exportPayload = useMemo(() => serializeNotificationHistoryPresetState(presetState), [presetState]);
  ...
  return (
    <ShellCard title="Preset transfer" subtitle="Import/export pinned and recent presets">
      <Text style={styles.meta}>Generate payload to share current preset state, or paste payload JSON to import.</Text>
      <TextInput
        multiline
        numberOfLines={7}
        value={payload}
        onChangeText={setPayload}
        placeholder="Paste preset transfer JSON here"
        autoCapitalize="none"
        autoCorrect={false}
