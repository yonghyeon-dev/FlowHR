...
- 근무일정/교대/유연근무 고도화(다직원 글로벌 fairness constraint 기반 최적화)
- 출퇴근 정책 고도화(다중 provider 연동/리스크 인텔리전스 운영 자동화)
- 실시간 근태 현황 고도화(리포트/알림/에스컬레이션 baseline 이후 대시보드)

### 진행 중

- 없음 (WI-0057까지 main에 머지 완료 기준)
...
**로드맵 반영**: RBAC 엔진 도입 완료(WI-0036). 이후 Phase 4(WI-0079~)에서 급여 규칙 엔진화.
...
| **근무일정** | ✅ 교대근무/유연근무 | ✅ 시차출근/재택 | ⚠️ WorkSchedule CRUD + template 단건/다건 + rotation assign/balance/optimize/fairness/fairness-apply baseline(WI-0040~0047, WI-0057, WI-0058, WI-0059, WI-0061); 다직원 글로벌 fairness constraint 최적화 미도입 | Medium |
| **출퇴근** | ✅ GPS/비콘/키오스크 | ✅ GPS/Wi-Fi/QR | ⚠️ 채널 메타데이터 + GPS/지오펜스/다중 사업장/디바이스 allowlist/attestation/anti-spoofing + signal fusion + dynamic reputation baseline(WI-0048~0056, WI-0060, WI-0062) 완료, 다중 provider 운영 자동화 미도입 | Medium |
