...
- 출퇴근 정책 고도화(지오펜스/채널 신뢰도/디바이스 검증)
- 실시간 근태 현황/이상 탐지(리포트 기반 -> 알림/자동화 단계)

### 진행 중

- 없음 (WI-0049까지 main에 머지 완료 기준)
...
| **출퇴근** | ✅ GPS/비콘/키오스크 | ✅ GPS/Wi-Fi/QR | ⚠️ 채널 메타데이터 + GPS 정책 baseline(WI-0048, WI-0049) 완료, 지오펜스/디바이스 신뢰도 미도입 | High |
