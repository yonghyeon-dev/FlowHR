...
- WI-0631 employee year-end input session-context productization (/employee/year-end-input removes manual organization/employee/token inputs, shows read-only session metadata, and gates API logs behind devtools + e2e-wi0631 regression guard)
- WI-0632 admin leave-calendar/leave-accrual session-context productization (/admin/leave-calendar and /admin/leave-accrual remove manual organization/admin/token inputs, derive session context metadata, and gate API logs behind NEXT_PUBLIC_FLOWHR_DEV_TOOLS + e2e-wi0632 regression guard)
- WI-0633 admin payroll-insurance session-context productization (/admin/payroll-insurance removes manual organization/admin/token inputs, adopts session context metadata display, and gates logs panel behind NEXT_PUBLIC_FLOWHR_DEV_TOOLS + e2e-wi0633 regression guard)
- WI-0634 employee root session-context productization (/employee root removes manual accessToken/sticky organization/employee context state, derives session context via useEmployeeRuntimeSession, and drops account-panel manual org/employee/token inputs + e2e-wi0634 regression guard)
- WI-0635 employee guide session-context productization (/employee/guide removes manual organization/employee/token inputs, derives context from Supabase session, and gates API logs behind NEXT_PUBLIC_FLOWHR_DEV_TOOLS + e2e-wi0635 regression guard)
- WI-0636 admin dashboard Korean copy normalization (/admin dashboard KO strings normalized from corrupted text to readable UTF-8 product copy while preserving dashboard/productization behavior + e2e-wi0636 regression guard)
...
- WI-0650 employee attendance prefill effect extraction + line-budget recovery (extract schedule-context correction prefill useEffect from /employee/page.tsx into page-attendance-prefill-effect hook, preserve auto-target selection behavior, and keep employee/page.tsx <=500 lines with e2e-wi0650 guard)
- WI-0651 admin approval-executions section file split (split page-sections.tsx into work-conditions / summary-escalation / queue section modules, keep each file <=300 lines, and add e2e-wi0651 split+line-budget guard)
- WI-0652 korean surface mojibake guard and people copy fix (fix corrupted ko copy in /admin/people title/action/kpi/log labels and add e2e-wi0652 guard for admin-people + withholding runtime labels + contracts runtime fallback labels)
- WI-0653 scheduling rotation fairness core helper extraction (extract template/employee normalization, global/advanced fairness constraint normalization, planned-minute/grade evaluators, and weekday-set helper from scheduling/service.ts into rotation-fairness-core-helpers.ts, keep behavior unchanged, and add e2e-wi0653 regression guard)
- WI-0654 withholding copy-runtime line-budget recovery (reduce src/components/withholding-receipt/copy-runtime.ts to stay within <=380 lines guard without behavior changes and add e2e-wi0654 regression guard)
- WI-0655 KO surface one-shot sweep and CI guard (expand KO guard coverage for withholding/payslips/contracts locale/runtime copy blocks, lock contract fallback strings + withholding activity fallback, and chain WI-0522/WI-0579/WI-0655 under test:e2e:ko-guard)
- WI-0656 scheduling rotation assignment core helper extraction (extract shared rotation assignment helpers for template resolution, weekday-set validation, overlap preflight, and schedule creation into rotation-assignment-core-helpers.ts; rewire service call sites with callback injection; add e2e-wi0656 guard; keep scheduling/service.ts <=2900 lines)
- WI-0657 leave promotion delivery history core helper extraction (extract recipient stats, delivery-history persistence, and retry recipient-id normalization into promotion-delivery-history-core-helpers.ts; rewire dispatch/retry call sites; add e2e-wi0657 guard; keep leave/service.ts <=2450 lines)

- WI-0658 approval execution escalation core helper extraction (extract execution stalled/priority comparator, escalation item projector, and webhook config+dispatch helpers into execution-escalation-core-helpers.ts; rewire approval/service.ts listing+escalation flows; add e2e-wi0658 guard; keep approval/service.ts <=2100 lines)
- WI-0659 admin approval executions stalled-risk summary UX hardening (/admin/approval-executions summary adds watch/critical stalled-risk tiers and max stalled-hours metric based on threshold-aware risk windows, plus KO runtime copy normalization for page/headline/status labels and workspace quick-jump labels with e2e-wi0659 regression guard)
- WI-0660 vercel production scope-context resolution root fix (vercel-production-deploy workflow now tries unscoped token context before configured/repository-owner scopes, validates token with vercel whoami, and emits context-specific access diagnostics to reduce false scope mismatch failures)

- WI-0661 scheduling rotation employee optimization helper extraction (extract employee-level rotation optimization orchestration into rotation-employee-optimization-helpers.ts for org-scope validation, period normalization, existing-schedule lookup, and ranked best-option selection; keep scheduling/service.ts delegation-only with e2e-wi0661 regression guard)
- WI-0662 scheduling editable-schedule overlap helper extraction (extract strict overlap filtering for create/update flows into schedule-overlap-helpers.ts and keep overlap conflict behavior unchanged while reducing service duplication with e2e-wi0662 regression guard)
- WI-0663 approval stage-history audit-payload helper extraction (extract stage-history listed payload plus escalation base/failure audit payload builders into approval/audit-payload-helpers.ts and rewire approval/service.ts call sites with e2e-wi0663 regression guard)
- WI-0664 approval policy/gate read-path helper decomposition (extract policy read-result resolver, gate preview actor-context resolver, preview view-model builder, and preview audit payload builder into approval/policy-read-helpers.ts; rewire previewApprovalPolicyGate/readApprovalPolicy with e2e-wi0664 regression guard)
- WI-0665 admin dashboard priority focus-cards UX (/admin adds priority queue section with severity-ranked focus cards and quick-jump links, backed by page-focus-cards helper for card construction + severity summary aggregation, with e2e-wi0665 regression guard)
- WI-0666 admin dashboard priority copy normalization guard (extract priority-focus ko/en copy into page-focus-copy.ts, rewire /admin focus section to locale helper APIs, and add e2e-wi0666 mojibake regression guard for priority copy module)
- WI-0667 approval escalation event-payload helper extraction (extract approval.execution.escalation.requested.v1 payload builder into execution-escalation-event-payload-helpers.ts and rewire publish call-site while preserving payload schema/item cap with e2e-wi0667 guard)
- WI-0668 approval execution-list audit payload helper extraction (add buildApprovalExecutionListedAuditPayload to approval/audit-payload-helpers.ts and rewire approval.execution.listed audit call-site with e2e-wi0668 regression guard)
- WI-0669 approval escalation input-helper extraction (extract escalation policy normalization and candidate filtering/sorting/limit selection into execution-escalation-input-helpers.ts and rewire triggerApprovalExecutionEscalation with e2e-wi0669 guard)
- WI-0670 approval escalation response-helper extraction (extract triggerApprovalExecutionEscalation response type/builder into execution-escalation-response-helpers.ts and rewire service return path with e2e-wi0670 guard)
- WI-0671 approval execution-list helper extraction (extract list option normalization and stalled-filter/priority-sort/limit selection pipeline into execution-list-helpers.ts; rewire listApprovalExecutions and keep audit behavior with e2e-wi0671 guard)
- WI-0672 approval stage-history list helper extraction (extract stage-history list limit normalization and trimmed query-input builder into stage-history-list-helpers.ts; rewire listApprovalStageHistory delegation and keep audit behavior with e2e-wi0672 guard)
- WI-0673 approval escalation message helper extraction (extract escalation webhook message composition into execution-escalation-message-helpers.ts and rewire triggerApprovalExecutionEscalation while preserving candidate cap/message shape with e2e-wi0673 guard)
- WI-0674 approval execution query helper extraction (extract list/escalation execution query-input builders into execution-query-helpers.ts; rewire listApprovalExecutions and triggerApprovalExecutionEscalation while preserving behavior with e2e-wi0674 guard)
- WI-0675 approval escalation event helper extraction (extract escalation requested event envelope+payload composition into execution-escalation-event-helpers.ts and rewire publish call-site while preserving event schema with e2e-wi0675 guard)
- WI-0676 scheduling list query helper extraction (extract listWorkSchedules permission-aware employee filter resolution into schedule-list-query-helpers.ts and rewire service call-site while preserving access semantics with e2e-wi0676 guard)
- WI-0677 approval list audit-entry helper extraction (extract stage-history/execution listed audit-entry composition into list-audit-entry-helpers.ts and rewire service append call-sites while preserving action/payload semantics with e2e-wi0677 guard)
- WI-0678 scheduling rotation-balance summary helper extraction (extract weekday aggregation/grade/recommendation computation into rotation-balance-report-helpers.ts and rewire listWorkScheduleRotationBalance while preserving report semantics with e2e-wi0678 guard)
- WI-0679 scheduling anomaly attendance-period helper extraction (extract anomaly report/cockpit attendance lookup window expansion into anomaly-attendance-period-helpers.ts and rewire both call-sites while preserving behavior with e2e-wi0679 guard)
- WI-0680 scheduling event-publisher context helper adoption (replace local scheduling event publisher resolver with shared resolveSchedulingEventPublisher and rewire publish call-sites while preserving behavior with e2e-wi0680 guard)
- WI-0681 scheduling anomaly side-effect context builder adoption (add reusable side-effect context builder in anomaly-side-effect-helpers.ts and replace duplicated inline context assembly in report/cockpit flows with e2e-wi0681 guard)
- WI-0682 scheduling anomaly window normalization helper adoption (add report/cockpit anomaly window normalization helpers in schedule-input-normalization-helpers.ts and rewire service call-sites while preserving behavior with e2e-wi0682 guard)
- WI-0683 scheduling rotation-balance audit payload helper extraction (extract rotation balance report audit payload builder into rotation-balance-report-helpers.ts and rewire listWorkScheduleRotationBalance audit append path with e2e-wi0683 guard)
- WI-0684 approval escalation audit-entry helper extraction (extract escalation generated/requested/failed/event_publish_failed audit-entry envelope builders into execution-escalation-audit-entry-helpers.ts and rewire triggerApprovalExecutionEscalation append paths with e2e-wi0684 guard)
- WI-0685 scheduling anomaly summary side-effect helper adoption (add emitAnomalySummarySideEffects to combine alert+escalation execution and rewire listScheduleAttendanceAnomalies call-site while preserving side-effect semantics with e2e-wi0685 guard)
- WI-0686 scheduling rotation-balance result helper extraction (extract rotation balance report response-model builder into rotation-balance-report-helpers.ts and rewire listWorkScheduleRotationBalance return path with e2e-wi0686 guard)
- WI-0687 scheduling write-actor context helper adoption (add resolveSchedulingWriteActorContext and rewire anomaly write flows to share actor+tenant resolution while preserving permission/tenant semantics with e2e-wi0687 guard)
- WI-0688 scheduling incidentId normalization helper extraction (extract incidentId trim/required validation into normalizeRequiredIncidentId in anomaly-service-context-helpers.ts and rewire lifecycle mutation path with e2e-wi0688 guard)
- WI-0689 approval escalation audit-actor context helper extraction (add shared escalation audit actor-context builder and rewire triggerApprovalExecutionEscalation audit append paths to reuse it with e2e-wi0689 guard)
- WI-0690 approval escalation error-message helper extraction (extract unknown-error normalization into toApprovalExecutionEscalationErrorMessage and rewire escalation failure audit payload paths with e2e-wi0690 guard)
- WI-0691 scheduling anomaly summary side-effect input helper extraction (extract summary side-effect input composition into anomaly-side-effect-helpers.ts and rewire listScheduleAttendanceAnomalies with e2e-wi0691 guard)
- WI-0692 scheduling anomaly ticket side-effect input helper extraction (extract cockpit ticket side-effect input composition into anomaly-side-effect-helpers.ts and rewire listScheduleAttendanceAnomalyCockpit with e2e-wi0692 guard)
- WI-0693 scheduling listInPeriod query helper extraction (extract listWorkSchedules data-access query-input builder into schedule-list-query-helpers.ts and rewire list path with e2e-wi0693 guard)
...
- WI-0756 prisma migration BOM encoding guard (remove UTF-8 BOM from 202603020001_wi0755_notices_db_read_model/migration.sql, add scripts/ci/check_prisma_migration_encoding.py + regression tests, and wire guard into CI contract-governance job to block recurrence)
- WI-0757 recruitment db persistence core journey (replace recruitment in-memory store with runtime DataAccess recruitment stores, add RecruitmentOpening/RecruitmentReferral prisma models + migration, and rewire recruitment API routes to async DB-backed opening/referral lifecycle with e2e-wi0757 regression guard)
- WI-0758 recruitment opening status lifecycle (add POST /api/recruitment/openings/{openingId}/status for admin/manager opening OPEN/CLOSED transition, wire /admin/recruitment opening rows with in-product close/reopen action, and keep employee recruitment opening query constrained to OPEN items via existing status filter + e2e-wi0758 regression guard)
- WI-0759 recruitment referral opening-status policy (block POST /api/recruitment/referrals on CLOSED openings with 409 opening_closed, enforce referral stage transition policy in POST /api/recruitment/referrals/{referralId}/stage, and align admin stage selector to allowed transitions with e2e-wi0759 regression guard)
- WI-0760 recruitment opening close guard (block POST /api/recruitment/openings/{openingId}/status CLOSED transition when active referrals exist via 409 pending_referrals, support force=true override for authorized close, and wire admin recruitment close action with guarded-confirm retry + e2e-wi0760 regression guard)
- WI-0761 recruitment active referral duplicate guard (block POST /api/recruitment/referrals when same opening+candidate email already has active referral via 409 duplicate_active, normalize email match trim/lowercase, and surface duplicate conflict guidance in employee recruitment submit flow + e2e-wi0761 regression guard)
- WI-0762 recruitment tenant boundary guard (enforce actor organization scope on openings/referrals list/create routes with organization_scope_mismatch rejection, and harden referral stage/withdraw routes to return not_found on cross-tenant access + e2e-wi0762 regression guard)
- WI-0763 admin analytics recruitment kpi panel (add recruitment snapshot panel to /admin/analytics with open openings, active referrals, and stalled 7d referrals from recruitment APIs; keep AdminKpiDashboard/AdminKpiSections within line-budget guards + e2e-wi0763 regression guard)
...
- WI-0769 admin analytics notices read KPI panel (add notices read coverage panel to /admin/analytics with published notices, no-read notices, and 3d+ unread aging counts from notices/read-receipts snapshot + e2e-wi0769 regression guard)
- WI-0770 admin onboarding contract template bootstrap (extend /admin/onboarding setup with active employment template readiness, load active template count from /api/contracts/templates, and add one-click ACTIVE default employment template bootstrap action + e2e-wi0770 regression guard)
- WI-0771 admin onboarding employee invite coverage (add GET /api/auth/invites audit-backed listing, wire /admin/onboarding invite coverage metrics + one-click pending employee invite dispatch, and expand onboarding checklist with invite completion state + e2e-wi0771 regression guard)
...
- WI-0804 admin dashboard core queue badge upgrade (`/admin` now aggregates approvals/payroll/contracts queue counters (total/critical/watch) and adds direct queue badges for approval-executions, payroll-close, and contracts with `e2e-wi0804` regression test)
- WI-0805 employee dashboard action priority badges (`/employee` priority-action panel now renders checklist-based urgency badges with remaining/total counts and direct section-jump actions, with `e2e-wi0805` regression test)
- WI-0806 admin notices status action visibility (`/admin/notices` list now shows edit/publish/delete controls consistently, locks published rows via disabled action state with lock reason, and adds draft/scheduled/published visibility counters + `e2e-wi0806` regression test)
- WI-0807 employee notices quick filters and summary visibility (/employee/notices now provides quick queue actions for all/unread/aging-risk with live counts and read-count-aware summary separators for faster acknowledgement triage + e2e-wi0807 regression test)
- WI-0808 employee notices initial auto load (/employee/notices now auto-loads published notices once session context is ready, while keeping manual refresh and existing read actions intact + e2e-wi0808 regression test)
- WI-0809 employee notices admin-link devtools gate (/employee/notices now shows /admin/notices shortcut only in devtools mode and keeps product mode free of admin shortcut exposure + e2e-wi0809 regression test)
- WI-0810 employee notices scoped mark-all read (/employee/notices mark-all action now scopes to visible unread notices when search/filter is active by passing optional noticeIds to /api/notices/read-all, while keeping global mark-all behavior as fallback + e2e-wi0810 regression test)
- WI-0811 admin notices initial auto-load + read-risk quick filter (/admin/notices now auto-loads once session context is ready and adds all/read-risk quick filter actions to triage published 0-read notices without extra API calls + e2e-wi0811 regression test)
- WI-0812 admin analytics notice priority action links (/admin/analytics notice panel now shows top-priority follow-up reason + direct no-read queue CTA and quick links; /admin/notices now applies deep-link query filters status/audience/q/risk on load with e2e-wi0812 regression test)
- WI-0813 admin dashboard summary split + benefits deeplink auto-load (extract summary compute/types from /admin dashboard to keep admin/page.tsx 335 lines under line budget, and apply status/risk/search deep-link filters with one-shot session-ready auto-load in /admin/benefits + e2e-wi0813 regression test)
- WI-0814 employee benefits deeplink filters + session auto-load (/employee/benefits now hydrates status/risk/search query params and performs one-shot session-ready auto-load for catalog/request data while keeping EmployeeBenefitsWorkspace within line-budget guard, with e2e-wi0814 regression test)
- WI-0815 mobile benefits deeplink shortcuts (Employee mobile home now adds dedicated benefits queue shortcuts for pending-aging risk and approved history, opening /employee/benefits with prefilled status/risk filters while keeping existing generic benefits shortcut and bridge compatibility, with e2e-wi0815 regression test)
- WI-0816 benefits DB persistence read model (replace benefits in-memory store path with runtime data-access backed persistence, add BenefitCatalogItem/BenefitRequest Prisma models + migration, wire memory/prisma data-access enefits store implementations, and update benefits API routes to await async store operations while preserving legacy baseline contracts, with e2e-wi0816 regression test)
- WI-0817 benefits catalog status lifecycle and inactive-request guard (add POST /api/benefits/catalog/{benefitId}/status for admin/manager ACTIVE/INACTIVE control, block POST /api/benefits/requests for inactive or organization-mismatched catalog items, reject non-SUBMITTED decision attempts with 409 invalid_state, and wire /admin/benefits catalog status controls + Korean copy normalization with e2e-wi0817 regression test)
- WI-0818 benefits history name-resolution with inactive catalog (employee benefits workspace now loads full catalog for history name-resolution while exposing ACTIVE-only requestable catalog in submit/list panels to keep inactive-item request blocking and preserve readable history labels, with e2e-wi0818 regression test)
- WI-0819 benefits catalog deactivate pending-request guard (block POST /api/benefits/catalog/{benefitId}/status INACTIVE transition when submitted requests remain, and show per-item submitted-count with disabled deactivate action on /admin/benefits catalog rows to prevent invalid lifecycle transitions, with e2e-wi0819 regression test)
- WI-0820 benefits request pending-priority sort (add optional GET /api/benefits/requests sort query with pending_priority ordering, prioritize submitted review queue by oldest requestedAt first, and wire /admin/benefits refresh path to load priority-ordered queue by default with e2e-wi0820 regression test)
- WI-0821 recruitment referral stalled-priority sort (add optional GET /api/recruitment/referrals sort query with stalled_priority ordering, prioritize active stalled referrals before terminal referrals, and wire /admin/recruitment refresh path to load stalled-priority queue by default with e2e-wi0821 regression test)
- WI-0822 admin recruitment deeplink filters + session auto-load (/admin/recruitment now hydrates stage/risk/q query filters on first render, performs one-shot session-ready workspace auto-load, and keeps AdminRecruitmentWorkspace line budget under 300 with e2e-wi0822 regression test)
- WI-0823 employee recruitment deeplink filters + session auto-load (/employee/recruitment now hydrates stage/risk/opening/q query filters on first render, performs one-shot session-ready workspace auto-load, and loads referral queue with sort=stalled_priority under line-budget guard with e2e-wi0823 regression test)
- WI-0824 admin analytics recruitment priority action links (/admin/analytics recruitment panel now resolves top-priority action by stalled/active/opening state, exposes recruitment quick-action deep links, and adds localized priority guidance copy with e2e-wi0824 regression test)
- WI-0825 mobile recruitment deeplink shortcuts (employee mobile home now adds stalled/submitted recruitment queue shortcuts and opens /employee/recruitment with risk=stalled_7d or stage=SUBMITTED filters, with e2e-wi0825 regression test)
- WI-0826 recruitment korean copy normalization (normalize corrupted ko runtime copy in admin/employee recruitment locale maps, fix duplicate-referral conflict message text, and replace broken summary separators in employee recruitment view with e2e-wi0826 regression test)
- WI-0827 employee dashboard korean copy and hub normalization (normalize corrupted ko runtime copy in employee account overview/workspace hub panels, and add direct links to pending benefits and stalled recruitment queues with e2e-wi0827 regression test)
- WI-0828 admin benefits pending-aging risk filter and dashboard communication hub deeplinks (add pending_3d admin benefits risk filter from URL normalization to queue filtering and selector copy, and update /admin workspace communication links to notices-benefits-recruitment risk queues with e2e-wi0828 regression guard) 
- WI-0829 admin analytics benefits priority action links (add top-priority action resolution and quick action links in AdminBenefitsKpiPanel for pending_3d and over_limit queues, plus localized benefits panel action/reason copy and e2e-wi0829 regression guard) 
- WI-0830 admin analytics onboarding priority action links (add top-priority action resolution and quick action links in AdminOnboardingKpiPanel for pending contract responses and invite coverage follow-up, plus localized onboarding panel action/reason copy and e2e-wi0830 regression guard) 
- WI-0831 admin analytics contract KPI panel (add contract lifecycle snapshot panel in /admin/analytics with decision queue, pending response, SLA overdue, and renewal-candidate metrics sourced from current-period contract document summary, plus localized panel copy and e2e-wi0831 regression guard) 
- WI-0832 admin analytics contract priority action links (extend AdminContractKpiPanel with top-priority action resolution and quick action links for sla-overdue, pending-response, and decision-queue follow-up, plus localized contract action/reason copy and e2e-wi0832 regression guard) 
- WI-0833 admin contracts deeplink filter hydration (hydrate /admin/contracts filter state from query keys q/status/expiresInDays/slaRisk/decisionQueueOnly/renewalCandidateOnly/nextStep on initial load, add reusable contract filter normalizers, and lock with e2e-wi0833 regression guard) 
- WI-0834 employee contracts deeplink filter hydration (hydrate /employee/contracts inbox filters from query keys q/status/deadline on initial load, add reusable employee inbox filter normalizers, and lock with e2e-wi0834 regression guard) 
- WI-0835 employee dashboard contract risk shortcuts (extend employee workspace hub documents links with /employee/contracts due-soon and overdue deeplink CTAs for faster contract response triage, with ko/en copy coverage and e2e-wi0835 regression guard) 
- WI-0836 admin dashboard contract risk links (extend admin workspace communication hub with contract SLA overdue and pending-response deep links, update ko/en communication labels to include contracts, and lock with e2e-wi0836 regression guard) 
- WI-0837 admin analytics contract focus deeplink refine (map contract focus metrics to filtered contract queues: decisionQueueOnly and slaRisk=OVERDUE while preserving analytics context handoff, with e2e-wi0837 regression guard) 
- WI-0838 admin dashboard workspace hub korean copy recovery (restore readable ko copy in /admin workspace hubs for approvals/people/worktime/payroll/communication sections, preserve contract risk queue links, and add e2e-wi0838 mojibake regression guard) 
- WI-0839 admin dashboard contract decision queue link (add /admin/contracts?decisionQueueOnly=true shortcut in /admin communication hub for ko/en locale labels while preserving existing SLA overdue and pending response links, with e2e-wi0839 regression guard) 
- WI-0840 employee dashboard contract action-needed shortcut (extend employee documents hub with /employee/contracts?status=pending_response quick jump in ko/en copy while preserving due-soon/overdue links, with e2e-wi0840 regression guard) 
- WI-0841 admin analytics contract KPI source-context links (append source=admin-analytics to contract panel priority/quick actions and add focusMetric context for decision-queue and SLA-overdue routes, with e2e-wi0841 regression guard) 
- WI-0842 admin contracts analytics source banner (show source-context banner on /admin/contracts when opened from /admin/analytics and map focusMetric query to readable contract queue label in ko/en copy, with e2e-wi0842 regression guard) 
- WI-0843 employee contracts source-context shortcuts (add source=employee-dashboard query to employee dashboard contract shortcuts for base/action-needed/due-soon/overdue routes while preserving existing status/deadline filters, with e2e-wi0843 regression guard) 
- WI-0844 employee contracts source entry banner (show source-entry hint in /employee/contracts when opened with source=employee-dashboard, keep ko/en localized hint copy, and guard inbox line budget with e2e-wi0844) 
- WI-0845 admin dashboard contract renewal-candidate link (add /admin/contracts?renewalCandidateOnly=true shortcut in admin communication hub with ko/en labels while preserving existing contract risk links, with e2e-wi0845 regression guard) 
- WI-0846 admin analytics contract renewal quick-action link (add renewal-candidate queue action to contract KPI panel and use renewalCandidateOnly route with analytics source context, plus ko/en copy key and e2e-wi0846 guard) 
- WI-0847 admin dashboard contract pending-response queue badge balance (add contractPendingResponseCount to admin summary aggregation from SENT documents and include it in /admin contract queue badge total/watch computation, with e2e-wi0847 regression guard) 
- WI-0848 admin dashboard contract queue subcount annotation (add queue breakdown annotation rows in /admin core queue badges, including contract decision/pending-response/SLA-overdue composition while preserving total/critical/watch calculations, with e2e-wi0848 regression guard) 
- WI-0849 admin dashboard contract queue subcount korean terminology lock (normalize ko contract queue breakdown labels from compact legacy tokens to spaced product terms, keep queue math unchanged, and add e2e-wi0849 regression guard)
- WI-0850 admin dashboard contract queue breakdown quick links (add subcount quick actions in contract queue badge to open decisionQueueOnly, status=SENT, and slaRisk=OVERDUE routes with admin-dashboard source context while keeping existing badge totals unchanged)
- WI-0851 admin contracts dashboard source banner (show dedicated source-entry banner in /admin/contracts when opened from admin-dashboard quick links and resolve focused queue label from decision/pending-response/sla-overdue filters while keeping admin-analytics banner behavior)
- WI-0852 admin approval queue dashboard deeplink hydration (add pending/stalled quick links in admin dashboard approval badge and hydrate /admin/approval-executions filters from query params with admin-dashboard source banner context)
- WI-0853 admin payroll queue dashboard source banner (add previewed/undistributed quick links in admin payroll queue badge and show admin-dashboard source banner with focused queue label in /admin/payroll-close)
- WI-0854 admin payroll delivery dashboard source banner (add undistributed-run shortcut from admin payroll queue badge to /admin/payroll-payslip-delivery and show admin-dashboard source banner with focused queue label in delivery workspace)
- WI-0855 admin dashboard queue open source context links (append source=admin-dashboard on approval/payroll/contracts queue open links in /admin so destination workspaces can render existing dashboard source-entry context)
- WI-0856 admin approval dashboard open-link stalled-filter fix: /admin/approval-executions opened from source=admin-dashboard without explicit stalledHoursMin hydrates threshold 0 to avoid implicit stalled-only queue; summary/list stalled risk baseline remains 24h.
- WI-0857 approval activity separator normalization (replace corrupted hardcoded separators in admin approval recent activity rows with locale-aware copy.summaryConnector reuse, removing mojibake-like tokens while keeping row status/history structure unchanged)
- WI-0858 admin workspace hub contract source context (append source=admin-dashboard at /admin workspace-hub render-time for contract risk/response/renewal links while preserving static hub definitions and existing contract dashboard-source banner behavior)
- WI-0859 admin communication hub source context (append source=admin-dashboard to /admin communication hub notices/benefits/recruitment links and render dashboard source-entry hints in destination workspace headers while preserving existing filters and queue behavior)
- WI-0860 admin analytics communication KPI source context links (append source=admin-analytics to /admin/analytics notices-benefits-recruitment priority/quick actions and surface analytics source-entry hints in destination admin workspaces while preserving existing queue filters and session-ready auto-load behavior, with e2e-wi0860 regression guard)
- WI-0861 admin analytics communication focus queue context (append focusMetric context to /admin/analytics notices-benefits-recruitment priority/quick links and show analytics source-entry hint with focused queue label in destination admin workspaces, while preserving existing filters and auto-load behavior, with e2e-wi0861 regression guard)
- WI-0862 admin communication analytics return action (add return-to-analytics header action in /admin/notices, /admin/benefits, and /admin/recruitment when entered via source=admin-analytics while preserving existing source hints and filter hydration behavior, with e2e-wi0862 regression guard)
...
- WI-0868 admin people session auto-load + source return (auto-refresh /admin/people once session context is ready, auto-sync selected employee history on selection/history-limit updates, add onboarding source propagation in readiness workspace links, and expose source-return action in people workspace header with e2e-wi0868 regression guard)
- WI-0869 employee session auto-load snapshot (/employee now runs one-shot session-ready snapshot refresh with runtime guard in production when bearer session is missing, and uses stable session-mode keying to avoid duplicate auto-load loops, with e2e-wi0869 regression guard)
- WI-0870 employee pay document source-context shortcuts (extend employee dashboard pay hub links with source=employee-dashboard for payslips/payslip-receipts/withholding-receipt, and add source-entry hint + back-to-dashboard action in payslip receipt and withholding receipt workspaces, with e2e-wi0870 regression guard)
- WI-0871 employee payslips dashboard source-entry (/employee/payslips now hydrates source=employee-dashboard, shows source-entry hint banner, and switches header return action label to dashboard-context copy while preserving default employee portal action outside source context, with e2e-wi0871 regression guard)
- WI-0872 employee contracts dashboard return action (/employee/contracts now resolves dashboard source context through a dedicated source-entry resolver, keeps the dashboard-entry hint copy, and exposes a source-scoped return-to-dashboard header action while preserving default behavior for non-dashboard entries, with e2e-wi0872 regression guard)
- WI-0873 employee notices dashboard source-entry (employee dashboard notices shortcut now appends source=employee-dashboard, and /employee/notices renders source-entry hint plus dashboard return action label while preserving existing notice filters and read actions, with e2e-wi0873 regression guard)
- WI-0874 employee benefits dashboard source-entry (employee dashboard benefits shortcut now appends source=employee-dashboard, and /employee/benefits renders source-entry hint plus dashboard return action label while preserving existing request filters and self-service submit/cancel flow, with e2e-wi0874 regression guard)
- WI-0875 employee recruitment dashboard source-entry (employee dashboard recruitment shortcut now appends source=employee-dashboard, and /employee/recruitment renders source-entry hint plus dashboard return action label while preserving referral filters and submit/withdraw flow, with e2e-wi0875 regression guard)
- WI-0876 employee schedule dashboard source-entry (employee dashboard schedule shortcut now appends source=employee-dashboard, and /employee/schedule renders source-entry hint plus dashboard return action label while preserving schedule filters/exports/attendance-correction CTA, with e2e-wi0876 regression guard)
- WI-0877 employee year-end input dashboard source-entry (employee dashboard year-end-input shortcut now appends source=employee-dashboard, and /employee/year-end-input renders source-entry hint plus dashboard return action label while preserving year-end simulation and validation workflow, with e2e-wi0877 regression guard)
- WI-0878 employee dashboard quick-action source-context (employee dashboard header quick actions for payslips and contracts now append source=employee-dashboard while preserving existing action ordering and labels, with e2e-wi0878 regression guard)
- WI-0879 admin dashboard line-budget recovery (extract admin queue-badge composition into page-queue-badges helper to restore /admin page under 360-line budget while preserving top-priority panel, workspace hub rendering, and admin-dashboard source-context queue links, with e2e-wi0879 regression guard)
- WI-0880 employee page line-budget recovery (compress orchestration/prefill/session wiring in /employee root to restore page.tsx under 500-line budget while preserving attendance prefill effect and mutation action bindings, with e2e-wi0880 regression guard)
- WI-0881 admin approval product-mode cleanup (apply production login-session gate to /admin/approval-policy, /admin/approval-templates, and /admin/approval-history by blocking actor-header fallback outside devtools, repair corrupted ko locale copy in approval policy/templates/history locale helpers, and add e2e-wi0881 regression guard)
- WI-0882 admin approval executions product-mode gate (apply production login-session requirement to /admin/approval-executions by blocking actor-header fallback outside devtools, keep devtools logs vs related-workspaces split, and add e2e-wi0882 regression guard)
- WI-0889 employee receipts production session gate and KO copy fix (enforce production login-session gate on /employee/payslip-receipts and /employee/withholding-receipt by blocking actor-header fallback outside devtools, expose /login guidance in product mode, and fix payroll insurance panel Korean session-label corruption, with e2e-wi0889 regression guard)
- WI-0890 employee account product copy simplification (remove role/org/bearer diagnostics from default /employee account summary, keep sign-in state + /login guidance product copy, and preserve detailed session/query diagnostics in devtools-only disclosure, with e2e-wi0890 regression guard)
- WI-0891 employee core workspaces production session gate and dev-link cleanup (apply production login-session gate to /employee/notices, /employee/benefits, /employee/recruitment, /employee/schedule, and /employee-guide by blocking actor-header fallback outside devtools, surface /login guidance in product mode, and hide employee-to-admin shortcut links unless NEXT_PUBLIC_FLOWHR_DEV_TOOLS is enabled, with e2e-wi0891 regression guard)
- WI-0892 employee pay self-service production session gate completion (apply production login-session requirement to /employee/payslips and /employee/year-end-input by gating actor-header fallback to devtools/non-production only, blocking refresh/download/finalized-load actions without bearer session, and showing /login guidance in product mode, with e2e-wi0892 regression guard)
- WI-0893 employee root production session gate completion (apply production login-session requirement to /employee root mutation runtime by gating actor-header fallback to devtools/non-production only, block attendance/leave snapshot actions without bearer session, and show /login guidance in dashboard chrome, with e2e-wi0893 regression guard)
- WI-0894 admin root production session gate alignment (apply product-mode requiresLoginSession guard on /admin root refresh path, restrict admin dashboard actor-header fallback to devtools/non-production only, and disable refresh CTA with /login guidance when bearer session is missing, with e2e-wi0894 regression guard)
