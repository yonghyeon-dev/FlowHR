  ...
  const normalizedPayload =
    payload && typeof payload === "object" && !Array.isArray(payload)
      ? {
          ...payload,
          deductionMode:
            (payload as Record<string, unknown>).deductionMode ?? "statutory_kr_baseline"
        }
      : payload;
  const parsed = previewPayrollWithDeductionsSchema.safeParse(normalizedPayload);
  if (!parsed.success) {
  ...
  try {
    ...
    const serviceInput =
      parsed.data.deductionMode === "profile"
        ...
        : parsed.data.deductionMode === "statutory_kr_baseline"
          ? {
              ...sharedInput,
              deductionMode: "statutory_kr_baseline" as const,
              statutory: parsed.data.statutory
            }
          : {
              ...sharedInput,
