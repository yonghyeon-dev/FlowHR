...
export default function EmployeePayslipsPage() {
  ...
  const compareWindowLabel = useMemo(() => {
    ...
    return isKoLocale ? `${selectedLabel} ?�牍?${compareLabel}` : `${selectedLabel} vs ${compareLabel}`;
          ...
          {showDevTools ? (
            <details className="details" style={{ marginTop: 12 }}>
              ...
              <p className="small">
                ...
                {stats.fail}) 路 {pageCopy.devTools.current} {pendingLabel ?? "-"}
              </p>
              {isProductionRuntime ? (
                <p className="small muted">
                  {pageCopy.devTools.session}:{" "}
                  {supabaseSession
                    ? `${supabaseSession.email ?? supabaseSession.userId} 路 role=${supabaseSession.role ?? "-"} 路 org=${supabaseSession.organizationId ?? "-"} 路 actor=${supabaseSession.actorId ?? "-"}`
                ...
                >
                  <span>
                    ...
                    <span className="muted">
                      {pageCopy.payslipList.gross} {formatKrw(run.grossPayKrw)} 路 {pageCopy.payslipList.deduction}{" "}
                      {formatKrw(run.totalDeductionsKrw)} 路 {pageCopy.payslipList.net} {formatKrw(run.netPayKrw)} 路{" "}
