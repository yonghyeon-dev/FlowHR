...
const ADMIN_WORKSPACE_HUB_DEFINITIONS: Record<AdminShellSectionKey, AdminWorkspaceHubDefinition> = {
  ...
  operations: {
    description: {
      ko: "근태, 휴가, 일정, 직원 대상 운영 흐름을 하나의 운영 레인에서 처리합니다.",
      en: "Handle attendance, leave, scheduling, and employee-facing operating workflows in one lane."
    },
    links: [
      { href: "/admin/operations", label: { ko: "운영 레인", en: "Operations lane" } },
      ...
      },
      {
        ...
        label: { ko: "복리후생 대기열", en: "Benefit backlog" }
      },
      {
        href: `/admin/recruitment?risk=stalled_7d&source=${ADMIN_HUB_SOURCE}`,
        label: { ko: "채용 지연 건", en: "Recruitment stalled" }
      },
      {
        href: "/admin/contracts?decisionQueueOnly=true",
        label: { ko: "계약 의사결정 대기", en: "Contract decision queue" }
      },
      {
  ...
  settingsAndReporting: {
    description: {
      ko: "알림, 보안, 기능 관리, 보고 기본값을 운영 설정으로 묶습니다.",
    ...
    links: [
      ...
      },
      {
        ...
        label: { ko: "연차 촉진 메일 설정", en: "Leave promotion email" }
      },
      {
      ...
      { href: "/admin/reports", label: { ko: "리포트", en: "Reports" } }
    ]
  }
};
