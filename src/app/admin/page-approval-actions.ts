...
type UseAdminApprovalQuickActionsInput = {
  firstPendingLeaveId: string;
  firstPendingAttendanceId: string;
  isKoLocale: boolean;
...
export function useAdminApprovalQuickActions({
  firstPendingLeaveId,
  firstPendingAttendanceId,
  isKoLocale,
  runtimeLocale,
  ...
  setLoadError,
  refreshSummary
}: UseAdminApprovalQuickActionsInput) {
  ...
  const runApprovalQuickAction = useCallback(
    async (queue: ApprovalQueue) => {
      ...
      const targetId =
        queue === "leave" ? firstPendingLeaveId : firstPendingAttendanceId;
      if (!targetId) {
        return;
      }
