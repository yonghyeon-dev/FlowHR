...
async function run() {
  ...
  assert.match(employeePage, /from "@\/app\/employee\/page-action-helpers";/);
  assert.match(employeePage, /await refreshEmployeeSnapshotFromHelper\(\{/);
  assert.match(employeePage, /await createAttendanceFromHelper\(\{/);
  assert.match(employeePage, /await checkOutNowFromHelper\(\{/);
  assert.match(employeePage, /await requestAttendanceCorrectionFromHelper\(\{/);
  assert.match(employeePage, /await createLeaveFromHelper\(\{/);
  assert.match(employeePage, /await cancelLeaveFromHelper\(\{/);
