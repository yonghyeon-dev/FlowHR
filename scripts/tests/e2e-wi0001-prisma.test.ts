...
async function run() {
  ...
  try {
    ...
    const invalidJsonRejectBody = await readJson<{ error: string }>(invalidJsonRejectResponse);
    ...
    const invalidPayloadRejectBody = await readJson<{ error: string }>(invalidPayloadRejectResponse);
