...
function run() {
  ...
  assert.match(
    filingConsole,
    /function buildSubmissionReviewMetaChips\(\s*submission: PayrollYearEndFilingSubmission,\s*compactAckDetail: boolean,\s*compactTransport: boolean,\s*compactFormat: boolean,\s*compactValidation: boolean,\s*compactResubmission: boolean\s*\)/
  );
