...
function run() {
  ...
  assert.match(
    filingConsole,
    /function buildSubmissionReviewMetaChips\(\s*submission: PayrollYearEndFilingSubmission,\s*compactAckDetail: boolean\s*\)/
  );
  ...
  assert.match(filingConsole, /return buildSubmissionReviewMetaChips\(submission, false\)\.join\(" · "\);/);
  assert.match(
    filingConsole,
