...
async function run() {
  ...
  assert.match(
    filingOpsDashboardSource,
    /id="filing-alert-response-guide"/,
    "ops dashboard should include filing alert response guide section"
  );
  assert.match(
    filingOpsDashboardSource,
    /aria-label="filing alert response guide list"/,
    "ops dashboard should expose alert response guide list container"
  );
  assert.match(
    filingOpsDashboardSource,
  ...
  assert.match(
    globalsCssSource,
    /\.alert-response-row/,
    "global styles should include alert response row style"
  );
  assert.match(
    globalsCssSource,
    /\.alert-owner-grid/,
    "global styles should include alert owner grid style"
  );
  assert.match(
    payrollApiSpec,
    /version:\s*1\.24\.0/,
  ...
  const resolveDrilldownModeFromAlertMetric = (filingOpsDashboardModule as unknown as {
    resolveDrilldownModeFromAlertMetric: (
      metric: "pending" | "rejected" | "validationFail" | "evidenceGap" | "timelineFailure"
    ) => "pending" | "rejected" | "validation_fail" | "evidence_gap" | "timeline_failure";
  }).resolveDrilldownModeFromAlertMetric;
  const buildFilingOpsAlertResponseRows = (filingOpsDashboardModule as unknown as {
    buildFilingOpsAlertResponseRows: (options: {
      alertRules: Array<{
        metric: "pending" | "rejected" | "validationFail" | "evidenceGap" | "timelineFailure";
        label: string;
        detail: string;
      ...
      ownerAssignments: Record<
        "pending" | "rejected" | "validationFail" | "evidenceGap" | "timelineFailure",
        { ownerRole: "payroll_operator" | "manager" | "admin"; ownerActorId: string }
      >;
    }) => Array<{
      metric: "pending" | "rejected" | "validationFail" | "evidenceGap" | "timelineFailure";
      level: "normal" | "watch" | "critical";
      ownerRole: "payroll_operator" | "manager" | "admin";
      ownerActorId: string;
      unassigned: boolean;
    }>;
  ...
  assert.equal(
    resolveDrilldownModeFromAlertMetric("validationFail"),
    "validation_fail",
    "alert metric should map to matching drilldown mode"
  );
  assert.equal(
    resolveDrilldownModeFromAlertMetric("timelineFailure"),
    "timeline_failure",
    "timelineFailure metric should map to timeline_failure mode"
  );

  const responseRows = buildFilingOpsAlertResponseRows({
    alertRules: [
      {
        metric: "pending",
    ...
    ownerAssignments: {
      pending: { ownerRole: "payroll_operator", ownerActorId: "PAY-QUEUE-OWNER" },
      rejected: { ownerRole: "manager", ownerActorId: "" },
      validationFail: { ownerRole: "manager", ownerActorId: "MGR-VALIDATION-GATE" },
      evidenceGap: { ownerRole: "payroll_operator", ownerActorId: "PAY-EVIDENCE-TRACE" },
      timelineFailure: { ownerRole: "admin", ownerActorId: "ADM-TIMELINE-IC" }
    }
  });

  assert.equal(responseRows[0]?.metric, "rejected", "critical alert rows should be prioritized first");
  assert.equal(
    responseRows[0]?.unassigned,
    true,
    "response row should mark owner as unassigned when actor id is empty"
  );
  assert.equal(
    responseRows[1]?.ownerActorId,
    "PAY-QUEUE-OWNER",
    "response row should keep assigned owner actor id"
  );
...
run()
  .then(() => {
    console.log(
      "e2e-wi0201-filing-ops-alert-response-guide-and-owner-assignment-baseline.test passed"
    );
  })
  .catch((error) => {
    console.error(error);
